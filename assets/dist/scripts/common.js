...
var gamanager;
(function (gamanager) {
    var GAManager = (function () {
        function GAManager() {
            this.ACCOUNTS = new util.StringMap();
            this.cm = new gamanager.ConnectionManager(this);
        }
        GAManager.prototype.initialize = function () {
            var _this = this;
            this.cm.authorize()
                .then(function () { return _this.cm.loadAnalytics(); })
                .then(function (param) { return _this.getAccountSummaries(param); })
                .then(function (param) { return _this.saveAccountSummaries(param); });
        };
        GAManager.prototype.getAccountSummaries = function (msg) {
        ...
        GAManager.prototype.saveAccountSummaries = function (data) {
            console.log('finální funce spuštena');
            var accounts = data.result.items;
            for (var i = 0; i < accounts.length; i++) {
                this.ACCOUNTS.put(accounts[i].id, new gamanager.Account(this, accounts[i].id, accounts[i].name, accounts[i].webProperties));
            }
            this.fillField(data.body);
        };
        GAManager.prototype.fillField = function (data) {
            var output = document.getElementById('query-output'), select = document.getElementById('accountSelect');
            ;
            for (var i = 0; i < this.ACCOUNTS.names().length; i++) {
                var key = this.ACCOUNTS.names()[i];
    ...
    gamanager.GAManager = GAManager;
})(gamanager || (gamanager = {}));

var gamanager;
(function (gamanager) {
    var ConnectionManager = (function () {
        function ConnectionManager(gam) {
            this.gam = gam;
            // Replace with your client ID from the developer console.
            this.CLIENT_ID = '1020473408261-7fg0f5kq946c8sgrgr22j4qi931v3eit.apps.googleusercontent.com';
            // Set authorized scope.
            this.SCOPES = ['https://www.googleapis.com/auth/analytics.readonly'];
        }
        ConnectionManager.prototype.authorize = function () {
        ...
        ConnectionManager.prototype.loadAnalytics = function () {
            var d = new Promises.Deferred();
    ...
    gamanager.ConnectionManager = ConnectionManager;
})(gamanager || (gamanager = {}));
...
(function (gamanager) {
    var Account = (function () {
        function Account(gam, _id, _name, webProperties) {
            this.gam = gam;
            this._id = _id;
            this._name = _name;
            this.WEB_PROPERTIES = new util.StringMap();
            for (var i = 0; i < webProperties.length; i++) {
                this.WEB_PROPERTIES.put(webProperties[i].id, new gamanager.WebProperty(this.gam, webProperties[i].id, webProperties[i].name, webProperties[i].internalWebPropertyId, webProperties[i].level, this._id, webProperties[i].profiles));
            }
        }
    ...
    gamanager.Account = Account;
})(gamanager || (gamanager = {}));

var gamanager;
(function (gamanager) {
    var WebProperty = (function () {
        function WebProperty(gam, id, name, internalId, level, accountId, profiles) {
            this.gam = gam;
            this.id = id;
            this.name = name;
            ...
            this.PROFILES = new util.StringMap();
            for (var i = 0; i < profiles.length; i++) {
                this.PROFILES.put(profiles[i].id, new gamanager.Profile(this.gam, profiles[i].id, profiles[i].name, profiles[i].type, this.accountId, this.id));
            }
        }
        return WebProperty;
    })();
    gamanager.WebProperty = WebProperty;
})(gamanager || (gamanager = {}));

var gamanager;
(function (gamanager) {
    var Profile = (function () {
        function Profile(gam, id, name, type, accountId, webPropertyId) {
            this.gam = gam;
            this.id = id;
            this.name = name;
            this.type = type;
...
///<reference path="./gamanager/GAManager.ts" />
...
///<reference path="./gamanager/entities/Profile.ts" />
///<reference path="./Promises/Promise.ts" />
///<reference path="./util/StringMap.ts" />
///<reference path="./typings/gapi.d.ts" />
...
window.onload = function () {
    var launcher = new Launcher(), button = document.getElementById('auth-button');
